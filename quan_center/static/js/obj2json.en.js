...
function ajaxBanner(ajax_url, ajax_data, prepend_location, func) {
	$.ajax({
		...
		if (ajax_url != "/message/private/") {
			alert("An error occurs when communicating with the server ...");
		}
	});
}
...
function banner_render(msg_type, id, content, no_prefix) {
	...
	if (no_prefix) {
		...
		switch (msg_type) {
			case 'error': 
			case 'warning': 
				zh_type = 'Warning: '; msg_type = 'warning'; break; 
			case 'success': 
				zh_type = 'Congratulations: '; msg_type = 'success'; break; 
			case 'info': 
			case 'primary': 
				zh_type = 'Hint: '; msg_type = 'info'; break; 
			default: zh_type = '';
		}
	}
...
function ajaxSlow(ajax_url, ajax_data, ajax_data_type, callback, interval) {
	...
	function rec(){
		...
		$.ajax({
			...
			if (ajaxSlow_log) {
				console.log("An error occurs when communicating with the server ...");
			}
			rec_life = 2;
		});
	}
